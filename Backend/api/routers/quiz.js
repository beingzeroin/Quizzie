...
router.patch("/enroll", checkAuth, checkAuthUser, verifyURL, async(req, res, next) => {
    ...
    Quiz.findById(req.body.quizId)
        ...
        .then(async(result2) => {
            ...
            await Quiz.updateOne({ _id: quizId }, { $push: { usersEnrolled: { userId } } })
                .exec()
                .then(async(result) => {
                    await User.updateOne({ _id: userId }, { $push: { quizzesEnrolled: { quizId } } })
                        .then(async(result1) => {
                            await res.status(200).json({
                                message: "Enrolled",
                            });
    ...
    async(req, res, next) => {
        ...
        Quiz.findOne({ quizCode: req.body.quizCode })
            ...
            .then(async(result2) => {
                ...
                const userId = req.user.userId;
                await Quiz.updateOne({ quizCode: req.body.quizCode }, { $push: { usersEnrolled: { userId } } })
                    .exec()
                    .then(async(result) => {
        ...
        .then(async(result) => {
            ...
            for (i = 0; i < numQuiz; i++) {
                if (result.quizzesEnrolled[i].quizId == req.body.quizId) {
                    ...
                    await User.updateOne({ _id: currentUser }, { $pull: { quizzesEnrolled: { quizId: req.body.quizId } } })
                        .then((result) => {
                            ...
                            await User.findById(req.user.userId)
                                .then(async(result2) => {
                                    ...
                                    await User.updateOne({ _id: req.user.userId }, { $push: { quizzesStarted: { quizId } } })
                                        .exec()
                                        .then(async(result1) => {
                                            await Quiz.updateOne({ _id: req.body.quizId }, { $set: { quizStatus: 1 } })
                                                .then(async(result1) => {
                                                    var data = [];
                                                    for (i = 0; i < result.length; i++) {
                                                        object = {
...
router.patch(
    ...
    async(req, res, next) => {
        ...
        console.log(flag)
        await Quiz.updateOne({ _id: req.body.quizId }, { $pull: { usersEnrolled: { userId: req.body.userId } } })
            .then(async(result) => {
                await User.updateOne({ _id: req.body.userId }, { $pull: { quizzesEnrolled: { quizId: req.body.quizId } } })
                    .then((result) => {
                        res.status(200).json({
